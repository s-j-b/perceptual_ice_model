...
public:
    ...
    SubscribeAndPublish() {
        ...
        const message_filters::Subscriber<sensor_msgs::Image> direct_sub_(nh_,
                                                                          base_topic_name + "/direct" + topic_type,
                                                                          q);
        const message_filters::Subscriber<sensor_msgs::Image> global_sub_(nh_,
                                                                          base_topic_name + "/global" + topic_type,
                                                                          q);
        const message_filters::Subscriber<sensor_msgs::Image> intensity_sub_(nh_,
                                                                             base_topic_name + "/intenisty" + topic_type,
                                                                             q);
        const message_filters::Subscriber<sensor_msgs::Image> depth_sub_(nh_,
                                                                         base_topic_name + "/depth" + topic_type,
                                                                         q);
        const message_filters::Subscriber<sensor_msgs::CameraInfo> cinfo_sub_(nh_,
                                                                              base_topic_name + "/left/camera_info", 
                                                                              q);
        message_filters::Synchronizer<SyncPolicy> sync_(SyncPolicy(q),
                                                        direct_sub_,
                                                        global_sub_,
                                                        intensity_sub_,
                                                        depth_sub_,
        ...
        sync_.registerCallback(boost::bind(&SubscribeAndPublish::callback, this, _1, _2, _3, _4, _5));
...
int main(int argc, char **argv) {
    ...
    message_filters::Subscriber<sensor_msgs::Image> direct_sub_;
    ...
    */
        ...
        direct_sub_ = message_filters::Subscriber<sensor_msgs::Image>(nh_,
                                                                      base_topic_name + "/direct" + topic_type,
                                                                      q);

        global_sub_ = message_filters::Subscriber<sensor_msgs::Image>(nh_,
                                                                      base_topic_name + "/global" + topic_type,
                                                                      q);

        intensity_sub_ = message_filters::Subscriber<sensor_msgs::Image>(nh_,
                                                                         base_topic_name + "/intenisty" + topic_type,
                                                                         q);

        depth_sub_ = message_filters::Subscriber<sensor_msgs::Image>(nh_,
                                                                     base_topic_name + "/depth" + topic_type,
                                                                     q);

        cinfo_sub_ = message_filters::Subscriber<sensor_msgs::CameraInfo>(nh_,
                                                                          base_topic_name + "/left/camera_info", 
                                                                          q);
...
void callback(ros::NodeHandle& nh,
    ...
    ros::Publisher pub = nh.advertise<sensor_msgs::PointCloud2>("dgi_points", 1000);
    pub.publish(pc2);
...
int main(int argc, char** argv) {
    ...
    const message_filters::Subscriber<sensor_msgs::Image> direct_sub(nh,
                                                               base_topic_name + "/direct" + topic_type,
                                                               q);
    const message_filters::Subscriber<sensor_msgs::Image> global_sub(nh,
                                                               base_topic_name + "/global" + topic_type,
                                                               q);
    const message_filters::Subscriber<sensor_msgs::Image> intensity_sub(nh,
                                                                  base_topic_name + "/intenisty" + topic_type,
                                                                  q);
    const message_filters::Subscriber<sensor_msgs::Image> depth_sub(nh,
                                                              base_topic_name + "/depth" + topic_type,
                                                              q);
    const message_filters::Subscriber<sensor_msgs::CameraInfo> cinfo_sub(nh,
                                                                   base_topic_name + "/left/camera_info", 
                                                                   q);

    message_filters::Synchronizer<SyncPolicy> sync(SyncPolicy(q),
                                                   direct_sub,
                                                   global_sub,
                                                   intensity_sub,
                                                   depth_sub,
